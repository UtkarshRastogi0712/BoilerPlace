...
export function activate(context: vscode.ExtensionContext) {
  ...
  let init = vscode.commands.registerCommand("boilerplace.init", async () => {
    ...
    if (vscode.workspace.workspaceFolders !== undefined) {
      ...
      wsedits.createFile(vscode.Uri.parse(origin + "/boilerplace.txt"));
      vscode.workspace.applyEdit(wsedits);
      fs.writeFile(origin.toString(), JSON.stringify(initFile), (err) => {
        vscode.window.showErrorMessage("Something went wrong");
      });
    } else {
      vscode.window.showErrorMessage("No workspace found");
    }
    vscode.workspace.fs.writeFile(origin, content);
  });
